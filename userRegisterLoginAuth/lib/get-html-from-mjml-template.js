...
export const getHtmlFromMjmlTemplate = async (template, data) => {
  ...
  // 1: we need to read tha data
  const mjmlTemplate = await fs.readFile(path.join(import.meta.dirname, "..", "emails", `${template}.mjml`),
    "utf-8"
  );
  ...
  // 3: we need to convert that fiule into html file
